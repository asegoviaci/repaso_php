...
class peliculas{
    ...
    public function anadirPelicula($pelicula){
        if (empty($pelicula->getNombre()) && empty($pelicula->getIsan())) {
            echo '<script>alert("Nombre y ISAN vacíos");</script>';
            return;
        }
        if (strlen($pelicula->getIsan()) !== 8) {
            echo '<script>alert("El ISAN debe tener 8 dígitos");</script>';
            return;
        }
        foreach ($this->array as $ISAN => $peli){
            if ($ISAN === $pelicula->getIsan()){
               
                if (empty($pelicula->getNombre())) {
                    unset($this->array[$ISAN]);
                } else {
